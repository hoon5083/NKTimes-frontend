...
function ArticleBrief({ article, boardId }: Props) {
  ...
  return condition ? (
    <Link href={url}>
      <div className="w-11/12 h-12 p-3 px-4 mx-auto my-1 rounded-lg cursor-pointer bg-cp-1 hover:shadow-xl">
        <div className="flex justify-between">
          <div className="text-lg font-semibold">{article.title}</div>
          <div className="flex justify-between">
            <div className="mr-4">{article.author.authority}</div>
            <div className="mr-4">{article.author.nickname}</div>
            <div className="mr-4 text-gray-500">
              {new Date(article.updatedAt).toLocaleDateString()}
            </div>
            <FontAwesomeIcon icon={faHeart} className="mt-1" size="sm" />
            <div className="mx-2 text-sm">{article.likeCount}</div>
          </div>
        </div>
